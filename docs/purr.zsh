...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  ...
  echo "ᓚᘏᗢ purr source loaded" | lolcat -a
fi
...
check_existing_setup() {
  ...
  if $has_existing; then
    echo "✨ Existing configuration found" | lolcat -a
    echo -e "\nCurrent setup:"
    echo -e "${details}"
...
purr() {
  ...
  lock)
    ...
    ;;
  "")
    echo "Starting key management..." | lolcat -a
    ...
    if load_ssh_key $verbose; then
      echo "[$current_step/$steps_total] SSH keys loaded successfully" | lolcat -a
    else
    ...
    if load_gpg_key $verbose; then
      echo "[$current_step/$steps_total] GPG keys loaded and agent configured" | lolcat -a
      if ! gpg --list-secret-keys $(git config --global user.signingkey) >/dev/null 2>&1; then
        echo "Reimporting GPG secret key..."
        if ! reimport_secret_key $verbose $(git config --global user.signingkey) $(op item get "gpg" --vault "purr" --field password --reveal); then
          echo "GPG key reimport failed"
          return 1
        fi
      fi
    else
      echo "GPG key loading failed"
      return 1
    fi
    ((current_step++))
    ...
    if enable_git_signing $verbose; then
      echo "[$current_step/$steps_total] Git signing configured successfully" | lolcat -a
    else
      echo "Git signing configuration failed"
      return 1
    fi
    ((current_step++))
    ...
    if setup_github_credentials $verbose; then
      echo "[$current_step/$steps_total] GitHub credentials configured successfully" | lolcat -a
    else
      echo "GitHub credentials setup failed"
      return 1
    fi

    echo "All keys loaded, agents configured, and Git/GitHub setup complete" | lolcat -a
...
setup_github_credentials() {
  ...
  if ! op item get "GitHub" --vault "purr" >/dev/null 2>&1; then
    echo "✗ GitHub item not found in 'purr' vault"
    return 1
  fi
